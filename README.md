...
Snake is a **verification tool**, not a discovery scanner. The load-bearing stage is turning a *candidate* into a *confirmed, evidenced finding*. Pair it with [aimap](https://github.com/sshpie/aimap) for the full discovery-to-finding pipeline.
...
[aimap](https://github.com/sshpie/aimap) fingerprints AI/ML services across 36 platform types. Snake reads aimap's JSON output and carries the service identification forward so it doesn't re-detect what aimap already confirmed.
